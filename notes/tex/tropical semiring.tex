...
\begin{mydefinition}{Tropical semiring}
    The tropical semiring is a special algebraic structure where the usual operations $+$ and $\times$ are replaced by the tropical addition $a \oplus b = \min(a, b)$ and the tropical multiplication $a \otimes b = a + b$. It is usually defined on $\overline{\mathbb{R}} = \mathbb{R} \cup \ \{\infty\}$ or $\overline{\mathbb{N}} = \mathbb{N} \cup \ \{\infty\}$ in computer science.
\end{mydefinition}
...
\textbf{Remark:} In this document we consider the min-plus algebra but everything is equivalent if we replace the min operation by a max operation (max-plus algebra).


\begin{mydefinition}{Semiring}
...
\begin{mytheorem}{Tropical semiring}
...
One of the most important property of the tropical semiring is \textbf{tropical matrix multiplication}. Tropical matrix multiplication is the same concept of ordinary matrix multiplication but the two scalar operations $+$ and $\times$ are replaced by the tropical operations $\min$ and $+$.
...
\begin{myproperty}{Associativity of Matrix Multiplication}
In both semirings, matrix multiplication is associative. It implies that we can compute the multiplication in any order.
\end{myproperty}
...
\begin{mydefinition}{Identity Matrix}
On the tropical semiring, the identity matrix is the matrix with $0$ on the diagonal and $\infty$ elsewhere.
\[
I_{n} = \begin{pmatrix}
0 & \infty & \cdots & \infty \\
\infty & 0 & \cdots & \infty \\
...
Some applications of the tropical semiring are \textbf{fast matrix exponentiation} and \textbf{range matrix multiplication}.
...
\begin{myexample}{Shortest path with exactly $k$ edges}
...
\textbf{Remark: } The problem is closely related to the Floyd-Warshall algorithm that computes the shortest distance between all pairs of nodes. The difference is that Floyd-Warshall can use any number of edges, whereas tropical matrix exponentiation uses exactly $k$ edges. Floyd-Warshall is generally used when the number of vertices is small, and tropical matrix exponentiation when the number of steps is very large and we care about the exact number of steps.
...
\begin{itemize}
    \item \href{https://cses.fi/problemset/task/1724/}{CSES 1724} - Graph Paths II
    \item \href{https://codeforces.com/contest/2257/problem/F1}{Codeforces 2257 F1} - Beaver's Jumping Track (Easy Version)
    \item \href{https://codeforces.com/problemset/problem/576/D}{Codeforces 576 D} - Flights for Regular Customers
    \item \href{https://codeforces.com/gym/102644/problem/F}{Codeforces 102644 F} - Min Path
\end{itemize}
